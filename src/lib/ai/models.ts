...
// 笔境 AI - 支持的 Nvidia 大模型列表
// ============================================================================

export interface ModelInfo {
  id: string;              // Nvidia model ID (用于API调用)
  name: string;            // 显示名称
  provider: string;        // 模型提供商
  description: string;     // 模型描述
  maxTokens: number;       // 最大上下文长度
  category: 'flagship' | 'balanced' | 'fast' | 'specialized';
  supportsVision?: boolean;
  supportsFunctionCall?: boolean;
}

/**
 * Nvidia NIM API 支持的主流模型列表
 * API文档: https://build.nvidia.com/explore/discover
 */
...
/** 默认模型（推荐用于小说创作） */
export const DEFAULT_MODEL_ID = 'nvidia/llama-3.1-nemotron-ultra-253b-instruct';
